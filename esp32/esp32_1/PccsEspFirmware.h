/*
 * PCCS5 ESP32-S3 lighting / analog MCU firmware (shared core).
 *
 ...
 *   PCCS_ESP_ID            1 or 2
 *   PCCS_PWM_PINS          {4,5,6,7,8,9,10,11}
 *   PCCS_ANALOG_PINS       {1} or {1,2}
 * Optional: PCCS_HOST_RX / PCCS_HOST_TX (defaults 17 / 18)
 */

#pragma once

#ifndef PCCS_ESP_ID
#error "Define PCCS_ESP_ID (1 or 2) before including PccsEspFirmware.h"
#endif

#ifndef PCCS_PWM_PINS
#error "Define PCCS_PWM_PINS before including PccsEspFirmware.h"
#endif

#ifndef PCCS_ANALOG_PINS
#error "Define PCCS_ANALOG_PINS before including PccsEspFirmware.h"
#endif

#ifndef PCCS_BAUD
#define PCCS_BAUD 115200
#endif

// Pi ↔ ESP UART1 (both ESP32-1 and ESP32-2 on the PCCS5 PCB).
#ifndef PCCS_HOST_RX
#define PCCS_HOST_RX 17
#endif
#ifndef PCCS_HOST_TX
#define PCCS_HOST_TX 18
#endif

#ifndef PCCS_PWM_FREQ_HZ
#define PCCS_PWM_FREQ_HZ 5000
#endif

#ifndef PCCS_PWM_RES_BITS
#define PCCS_PWM_RES_BITS 8
#endif

#ifndef PCCS_ADC_SAMPLES
#define PCCS_ADC_SAMPLES 64
#endif

// 3V3 rail report for water-level math on the Pi (mV).
#ifndef PCCS_VCC_MV
#define PCCS_VCC_MV 3300
#endif

#include <Arduino.h>

static const int kPwmPins[] = PCCS_PWM_PINS;
static const int kNumPwm = sizeof(kPwmPins) / sizeof(kPwmPins[0]);
static const int kAnalogPins[] = PCCS_ANALOG_PINS;
...
static float readAnalogAvg(int gpio) {
  long sum = 0;
  for (int i = 0; i < PCCS_ADC_SAMPLES; i++) {
    sum += analogRead(gpio);
    delayMicroseconds(50);
  }
  return static_cast<float>(sum) / static_cast<float>(PCCS_ADC_SAMPLES);
}
...
static void processGetVcc() {
  ...
  HostSerial.println(PCCS_VCC_MV);
}
...
static void pccsSetup() {
  ...
  for (int i = 0; i < kNumPwm; i++) {
    ...
    analogWriteResolution(gpio, PCCS_PWM_RES_BITS);
    analogWriteFrequency(gpio, PCCS_PWM_FREQ_HZ);
#else
    analogWriteResolution(PCCS_PWM_RES_BITS);
#endif
  ...
  HostSerial.begin(PCCS_BAUD, SERIAL_8N1, PCCS_HOST_RX, PCCS_HOST_TX);
}

static void pccsLoop() {
  servicePwmRamps();
