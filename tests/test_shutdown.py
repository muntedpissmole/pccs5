...
class ShutdownApiTests(unittest.TestCase):
    ...
    def test_api_shutdown_powers_off_screens_before_host(self):
        ...
        with patch.object(pccs_app, "runtime") as runtime:
            runtime.screen_actuator = screen_actuator
            with patch.object(pccs_app, "_issue_host_shutdown", side_effect=record_host):
                with patch.object(pccs_app, "_cleanup_best_effort", side_effect=record_cleanup):
                    with patch("time.sleep"):
                        client = pccs_app.app.test_client()
