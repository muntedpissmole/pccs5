...
#   sudo ./install.sh --update     # git pull + deps (keeps pccs.conf)
...
#
# Safe to re-run: never replaces an existing config/pccs.conf; only updates
...
logo() {
  ...
  ╚═╝       ╚═════╝  ╚═════╝ ╚══════╝
EOF
    echo "${C_RESET}${C_DIM}     Pissmole Camper Control System${C_RESET}"
    echo "${C_DIM}     ──────────────────────────────${C_RESET}"
    echo
}
...
flash_one_esp() {
    ...
    while true; do
        ...
        echo "    ${C_DIM}(programming USB connected to the PCCS PCB)${C_RESET}"
        echo
        echo "  ${C_YELLOW}!${C_RESET} No ESP / PCCS core on the bench? Type ${C_BOLD}s${C_RESET} to skip this module."
...
step_esp() {
    ...
    echo
    warn "Only needed when the PCCS core / ESP32 modules are present."
    ...
    if [[ "$mode" == "optional" ]]; then
        info "Needs the PCCS PCB, programming USB, and both ESP32 modules."
...
EOF
    ...
    ssh -t "${SSH_BASE[@]}" -i "${KEY}" "${SCREEN_USER}@${SCREEN_HOST}" \
        "sudo bash -c $(printf '%q' "$remote_cmd")"
    echo "Verifying passwordless sudo from PCCS..."
...
PY
    ...
    if [[ ${#SCREEN_ROWS[@]} -eq 0 ]]; then
        warn "No screens in [screens] — add host/user (and mac) in pccs.conf first"
        if [[ "$mode" == "optional" ]]; then
...
step_checklist() {
    step_begin "Notes"
    info "Touchscreens: set [screens] in pccs.conf, then menu item or --screens"
    info "Hardware you skipped: use this menu anytime; pccs.conf is never replaced"
    ok "Done"
}
...
step_update() {
    ...
    # Never: cp pccs.conf.dist → pccs.conf
    [[ -f "$CONF" ]] && ok "config/pccs.conf still present and untouched"
...
run_install() {
    ...
    info "Existing pccs.conf is never replaced."
    echo
...
${C_BOLD}PCCS5 setup${C_RESET}
  ...
  sudo $0 --update        Pull latest code (keeps pccs.conf)
