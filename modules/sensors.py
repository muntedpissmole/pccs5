...
class SensorManager:
    def __init__(self, config, send_command_func, socketio):
        ...
        # Dual 1-Wire buses (PCCS PCB): GPIO4 outside+fridge, GPIO3 freezer.
    ...
    def _discover_w1_masters_by_gpio() -> dict[int, str]:
        ...
        if not masters:
            ...
            if len(masters_found) == 1 and len(gpio_to_dt) == 1:
                ...
                # Single w1 master without DT pin map — default GPIO for PCCS5.
            ...
            if sensor_id:
                ...
                if not os.path.isdir(device_folder):
                    ...
                    if now - last > 60:
                        ...
                        logger.warning(
                            "   → Update [sensors] %s_temp_sensor in config/pccs.conf with the correct ID",
    ...
    def _calculate_level_percent(self, adc, vcc, r_empty, r_full, r_pullup=None, adc_max=None):
        ...
        PCCS5: R_pullup = 150 Ω to 3V3 on each analog input.
