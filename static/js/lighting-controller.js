/**
 * PCCS5 Lighting tab — sliders and relays via Socket.IO + HTTP fallback.
 */
(function () {
    'use strict';
    ...
    function emitLightChange(payload) {
        ...
        fetch('/api/light', {
            ...
            .catch((err) => console.warn('[PCCS5] light_change HTTP failed', err));
    }
    ...
    function emitRelayChange(name, on) {
        ...
        fetch('/api/relay', {
            ...
            keepalive: true,
        }).catch((err) => console.warn('[PCCS5] relay_change HTTP failed', err));
    }
    ...
    function updateLightUI(name, value) {
        ...
        if (light.type === 'relay') {
            ...
            window.PCCS5.lightingHome?.onLightUpdate?.(name, isOn ? 1 : 0);
            return;
        }
        ...
        window.PCCS5.lightingHome?.onLightUpdate?.(
            name,
    ...
    window.PCCS5.lighting = {
        ...
        setSceneActivating(value, rampMs) {
            ...
            if (rampMs) window.PCCS5._sceneRampMs = rampMs;
        },
        setReedActivating(value) {
            reedActivating = !!value;
        },
        getSceneRampMs() {
            return window.PCCS5._sceneRampMs || SCENE_RAMP_MS;
        },
