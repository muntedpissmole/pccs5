/**
 * PCCS5 Toast test tile — preview composer + server broadcast via socket.
 */
(function () {
    'use strict';
    ...
    function updateStats() {
        const active = window.pccs5Toasts?.getActiveCount?.() ?? 0;
    ...
    function sendToast() {
        ...
        const socket = window.PCCS5?.socket;
        if (socket?.connected) {
            socket.emit('toast_test', payload);
        } else if (window.pccs5Toasts?.create) {
            window.pccs5Toasts.create(payload);
        }
