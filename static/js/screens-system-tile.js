/**
 * PCCS5 Touchscreens tile — live status via REST + manual wake/sleep via Socket.IO.
 */
(function () {
    'use strict';
    ...
    function setScreenPower(name, awake) {
        ...
        if (!socket?.connected) {
            console.warn('[PCCS5] screen_manual_toggle skipped — socket unavailable', name);
            return;
        }
    ...
    function setScreenBrightness(name, pct) {
        ...
        const socket = getSocket();
        if (!socket?.connected) {
            console.warn('[PCCS5] screen brightness skipped — socket unavailable', name);
            return;
        }
    ...
    async function refreshAll() {
        ...
        try {
            ...
            console.warn('[PCCS5] screen status fetch failed', err);
        }
    }
    ...
    async function testScreen(name) {
        ...
        try {
            ...
            console.warn('[PCCS5] screen test failed', name, err);
    ...
    async function loadScreens() {
        ...
        try {
            ...
            console.warn('[PCCS5] screen config fetch failed', err);
        }
    }

    window.PCCS5.screensSystem = {
