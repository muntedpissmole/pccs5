/**
 * PCCS5 Sonos tile — transport, mute, volume display, now playing (socket + HTTP fallback).
 */
(function () {
    'use strict';
    ...
    function unavailableMessage(data) {
        ...
        if (data?.source === 'disabled') return 'Sonos disabled in configuration';
    ...
    function setUnavailable(data) {
        ...
        window.PCCS5?.homeModuleVisibility?.setSonosOnline?.(false);
    }

    function setAvailable() {
        tile?.classList.remove('is-unavailable');
        setControlsDisabled(false);
        window.PCCS5?.homeModuleVisibility?.setSonosOnline?.(true);
    }
    ...
    function isSonosOnline(data) {
        ...
        // Empty speakers list means discovery found nothing (hardware offline).
    ...
    if (socket) {
        ...
        document.addEventListener('pccs5:socket-ready', (event) => {
