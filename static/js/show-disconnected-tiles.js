/**
 * PCCS5 "Show tiles with disconnected hardware" preference.
 * Local-only (per-device) toggle — no server sync.
 */
(function () {
    'use strict';

    const STORAGE_KEY = 'pccs5-show-disconnected-tiles';
    ...
    function apply(value, { persist: shouldPersist = true } = {}) {
        ...
        window.PCCS5?.homeModuleVisibility?.setShowDisconnected?.(value);
        window.PCCS5?.lighting?.setShowDisconnected?.(value);
        window.PCCS5?.scenes?.setShowDisconnected?.(value);
    }
