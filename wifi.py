...
def _build_uplink(wifi_status: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "active": active,
        "choice_available": wifi_connected and usb_connected,
...
def set_preferred_uplink(prefer: str) -> dict[str, Any]:
    """Prefer Wi‑Fi or USB tether for the default IPv4 route (route metric)."""
    ...
    wifi_path = uplink.get("wifi")
    usb_path = uplink.get("usb")

    if not wifi_path or not wifi_path.get("connected"):
        return {
            "ok": False,
            "error": "Wi‑Fi is not connected",
            "status": status,
        }
    if not usb_path or not usb_path.get("connected"):
        return {
            "ok": False,
            "error": "USB tether is not connected",
            "status": status,
        }

    wifi_con = wifi_path.get("connection")
    usb_con = usb_path.get("connection")
    wifi_iface = wifi_path.get("iface")
    usb_iface = usb_path.get("iface")

    if not wifi_con or not usb_con:
        return {
            "ok": False,
            "error": "Could not resolve NetworkManager connection profiles",
            "status": status,
        }
    if not wifi_iface or not usb_iface:
        return {
            "ok": False,
            "error": "Missing network interfaces for preference switch",
            "status": status,
        }

    if prefer == "usb":
        preferred_con, preferred_iface = usb_con, usb_iface
        fallback_con, fallback_iface = wifi_con, wifi_iface
        label = "USB tether"
    else:
        preferred_con, preferred_iface = wifi_con, wifi_iface
        fallback_con, fallback_iface = usb_con, usb_iface
        label = wifi_path.get("ssid") or "Wi‑Fi"

    ok_pref, err = _set_connection_metric(preferred_con, _METRIC_PREFERRED)
    if not ok_pref:
        return {"ok": False, "error": err, "status": get_wifi_status()}

    ok_fb, err = _set_connection_metric(fallback_con, _METRIC_FALLBACK)
    if not ok_fb:
        return {"ok": False, "error": err, "status": get_wifi_status()}

    _reapply_connection(str(preferred_iface), str(preferred_con))
    _reapply_connection(str(fallback_iface), str(fallback_con))

    status = get_wifi_status()
    active = (status.get("uplink") or {}).get("active")
    if active != prefer:
        # Metrics may need a moment; report success with a note if route lags.
        return {
            "ok": True,
            "message": f"Preferred internet set to {label} (route updating…)",
            "status": status,
        }

    return {
        "ok": True,
        "message": f"Internet via {label}",
        "status": status,
    }