"""PCCS5 — Flask + SocketIO bridge for lighting/relay control."""
...
def _patch_engineio_werkzeug_websocket() -> None:
    ...
    if getattr(wswsgi.SimpleWebSocketWSGI, "_pccs5_werkzeug_patch", False):
        return
    ...
    wswsgi.SimpleWebSocketWSGI._pccs5_werkzeug_patch = True


_patch_engineio_werkzeug_websocket()

app = Flask(__name__)
app.config["SECRET_KEY"] = config.get("system", "secret_key", fallback="pccs5-secret")
CORS(app)
...
def _startup():
    ...
    logger.info("Starting PCCS5 lighting backend...")
    ...
    logger.info("PCCS5 lighting backend ready")
...
if __name__ == "__main__":
    ...
    try:
        ...
        threading.Thread(target=_run_startup, name="pccs-startup", daemon=True).start()
        socketio.run(
            app,
            host=host,
            port=port,
